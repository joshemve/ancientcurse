...
public class SandstormTornado {
    ...
    // Increase rotation speed at higher levels: multiplier applied per tick
    ...
    // Reduced ENTITY_SPIN_SPEED for more controlled spin (from 45.0 to 15.0 degrees per tick)
    ...
    private static final int SPIN_DURATION = 60;   // 3 seconds spin (increased from 30)
    private static final int THROW_DURATION = 20;  // 1 second throw (increased from 10)
    private static final double MAX_LIFT_HEIGHT = 10.0; // Increased from 5.0 for more dramatic effect
    // Reduced THROW_STRENGTH for a more controlled throw (from 2.5 to 2.0)
    private static final double THROW_STRENGTH = 2.0;
    private static final int FADE_START_TICK = 50;
    ...
    public void tick(MinecraftServer server) {
        ...
        // Handle entity movement phases (without glow effects)
        if (target != null && target.isAlive()) {
            ...
            if (age <= FREEZE_DURATION) {
                ...
                // Trigger lightning when the entity is closer to the ground
                // Removed lightning strike logic
            }
        }

        // Increase overall rotation with extra acceleration (faster spin as time increases)
        ...
        if (world instanceof ServerWorld serverWorld) {
            ...
            if (age % 2 == 0) { // spawn every other tick
    ...
    // ---------- Entity Phase Methods (Enhanced for Godweapon Effect) ----------

    // Freeze Phase: Immobilize and add a swirling dust aura (glow removed)
    ...
    // Lift Phase: Gradually raise the entity (glow removed)
    ...
    // Spin Phase: Rotate the entity more dramatically
    private void spinEntity() {
        if (target != null) {
            ...
            double orbitRadius = 2.0 * (1.0 + easedSpin); // Gradually increase orbit radius
            ...
            target.setPos(x, y, z);
            target.setYaw((float)spinAngle % 360); // Make the entity visually spin
            target.prevYaw = target.getYaw(); // Prevent jerky rotation
    ...
    // Throw Phase: Propel the entity more rapidly with an explosive burst
    private void throwEntity() {
        if (target != null) {
            ...
            double upwardForce = Math.max(0, 1.0 - throwProgress * 2) * 0.8; // Initial upward boost
            ...
            // Spawn particles and strike lightning at the peak of the throw
            if (world instanceof ServerWorld serverWorld) {
