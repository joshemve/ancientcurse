 ...
 * 1. All blocks should be registered through specialized registry classes
 * (e.g., NecrostoneBlocks, CursedPlantBlocks)
 * 2. Each registry class should handle its own block type and follow the same
 * pattern:
 ...
 * 3. DO NOT register blocks directly in ModBlocks.java - use the appropriate
 * registry class
 * 4. When adding a new block type, create a new registry class and add it to
 * this central registry
 * 5. Always update both registerAll() and registerBlockItems() methods when
 * adding a new registry class
 * 
 * This approach prevents duplicate registrations and makes the codebase more
 * maintainable.
 */
